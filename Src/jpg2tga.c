...
// subsampling factor).
uint8 *pjpeg_load_from_file(uint8_t *pFilename, int *x, int *y, int *comps, pjpeg_scan_type_t *pScan_type, int reduce, uint8_t *ptr_picture )
{
   ...
   *comps = 0;
   ...
   g_pInFile = fmemopen(pFilename,malloc_usable_size(ptr_picture), "rb");
   if (!g_pInFile)
      return NULL;
